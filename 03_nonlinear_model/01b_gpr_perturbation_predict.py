# -*- coding: utf-8 -*-
"""
文件名: 01b_gpr_perturbation_predict.py
(注意: 此脚本应放置在 03_nonlinear_model 文件夹中)

功能: 
    1. 加载 01a 脚本训练好的 GPR (Tuned) Pipeline (Scaler + GPR)。
    2. 加载 05 (01_read_data) 脚本生成的扰动数据集。
    3. [参照02b] 清洗扰动数据，移除所有特征列均为空(NaN)或0的行。
    4. 使用 GPR Pipeline 对*清洗后*的数据集进行批量预测。
    5. 保存详细的原始预测结果 (ICE)。
...
# 模型输入/输出路径: .../steel/03_nonlinear_model/results/01_gpr_tuned_results/
MODEL_IO_DIR = 'results/01_gpr_tuned_results'
...
# --- 模型配置 ---
MODEL_NAME_PREFIX = '01_GPR_Tuned' # 必须与 01a 脚本中保存的模型名前缀一致
...
def load_models(model_dir, metrics, model_name_prefix):
    ...
    for metric in metrics:
        ...
        try:
            ...
            print(f"    请确保已运行 01a_gpr_tuning.py 脚本并成功保存了模型。")
            return None
...
def main():
    """主执行函数"""
    print("=" * 60)
    print("--- 启动脚本: 01b_gpr_perturbation_predict.py  ---")
    ...
    for feature_name in PERTURBED_FEATURES_LIST:
        ...
        print(f"\n{'='*20} 正在处理: {feature_name} (GPR Tuned) {'='*20}")
        ...
        # 2b. (任务一) 批量预测
        print("    正在执行批量预测 (GPR Pipeline)...")
        print("    (注意: GPR预测速度较慢，请耐心等待...)")
        
        for metric, model in tqdm(models.items(), desc="    预测性能 (GPR)"):
            ...
            if X_predict.isnull().values.any():
                ...
                print("    GPR无法处理。将使用 0 填充剩余NaN值后继续...")
                # 使用0填充。
        ...
        # 2c. 保存原始预测结果 (ICE)
        output_ice_filename = f'01b_gpr_prediction_results_{safe_feature_name}.csv'
        ...
        # 2e. 保存统计汇总结果 (PDP)
        output_pdp_filename = f'01b_gpr_summary_stats_{safe_feature_name}.csv'
        ...
        for metric in PERFORMANCE_METRICS:
            ...
            plot_title = f'{feature_name} 扰动 对 {metric} 的影响 (PDP - GPR Tuned)'
