...
功能: 
    ...
    2. 对每个性能指标，使用设置了随机种子的K-折交叉验证来评估 XGBoost 模型的性能。
...
OUTPUT_DIR = 'results/02_xgboost_results' # 更改为XGBoost的特定输出目录
...
def main():
    """主执行函数"""
    print("=" * 60)
    print("--- 启动脚本: 02a_xgboost_modeling.py ---")
    print("=" * 60)
    ...
    for metric in Y_all.columns:
        print(f"\n\n{'='*20} 正在为性能指标: '{metric}' 进行建模 (XGBoost) {'='*20}")
        ...
        # 这是为了防止XGBoost内部存储格式问题，即使不用SHAP也保留
        ...
        model = xgb.XGBRegressor(
            ...
            n_jobs=-1,
            objective='reg:squarederror', # 指定回归任务
            booster='gbtree',
        ...
        # 特征重要性 (XGBoost 同样支持 .feature_importances_)
        ...
        plt.title(f'"{metric}" 的Top 20重要特征 (XGBoost)', fontsize=16)
    ...
    if cv_performance_summary:
        ...
        print(f"\n\n--- XGBoost 交叉验证性能汇总表 ({K_FOLDS}-Fold CV, random_state={RANDOM_STATE}) ---")
        print(summary_df.to_string(index=False))
        
        # 更改汇总文件名
        summary_path = os.path.join(OUTPUT_DIR, 'xgboost_performance_summary.csv')
        summary_df.to_csv(summary_path, index=False, encoding='utf-8-sig')
        print(f"\n交叉验证性能汇总表已保存至: {summary_path}")

    print("\n" + "=" * 60)
    print("--- 步骤2a (XGBoost) 全部任务完成 ---")
    print("=" * 60)
