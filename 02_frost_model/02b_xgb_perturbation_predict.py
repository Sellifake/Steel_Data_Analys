# -*- coding: utf-8 -*-
"""
文件名: 02b_xgb_perturbation_predict.py
功能: 
    1. 加载 02a 脚本训练好的 XGBoost (XGB) 模型。
    2. 加载 05 脚本生成的扰动数据集 (perturbed datasets)。
    3. 使用 XGB 模型对扰动数据集进行批量预测。
...
import re
import joblib
import xgboost as xgb # 导入xgb，确保joblib反序列化时能找到库
...
# --- 路径配置 ---
# 假设此脚本位于 .../steel/02_frost_model/
# 模型输入/输出路径: .../steel/02_frost_model/results/02_xgboost_results/
MODEL_IO_DIR = 'results/02_xgboost_results'
...
def load_models(model_dir, metrics):
    ...
    for metric in metrics:
        ...
        try:
            ...
            # 更新错误提示
            print("    请确保已运行 02a_xgboost_modeling.py 脚本并成功保存了模型。")
            return None
...
def main():
    """
    主执行函数
    使用XGBoost模型对扰动数据进行预测分析和可视化
    """
    print("=" * 60)
    print("--- 启动脚本: 02b_xgb_perturbation_predict.py ---")
    print("--- 目的: 使用 XGB 模型对扰动数据进行预测、统计与可视化 ---")
    print("=" * 60)
    ...
    for feature_name in PERTURBED_FEATURES_LIST:
        ...
        for metric, model in tqdm(models.items(), desc="    预测性能"):
            ...
            # 执行预测 (XGBoost的 predict 接口与 sklearn 一致)
        ...
        # 更改输出文件名
        output_ice_filename = f'02b_xgb_prediction_results_{safe_feature_name}.csv'
        ...
        # 2e. 保存统计汇总结果
        # 更改输出文件名
        output_pdp_filename = f'02b_xgb_summary_stats_{safe_feature_name}.csv'
        ...
        for metric in PERFORMANCE_METRICS:
            ...
            # 设置图表标题和标签
            plot_title = f'{feature_name} 扰动 对 {metric} 的影响 (PDP - XGBoost)'
